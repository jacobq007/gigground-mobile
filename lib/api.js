...
import { compatibility, verifiedSkillKeys, openToKeys } from "./skills";

const delay = (ms = 320) => new Promise((r) => setTimeout(r, ms));
const clone = (x) => JSON.parse(JSON.stringify(x));
const uid = (p = "x") => p + "_" + Math.random().toString(36).slice(2, 9);
const DB_VERSION = "v8"; // bump when seed data changes to force a reset
...
async function ensure() {
  ...
  if (!db) {
    db = {
      ...
      matches: clone(seed.SEED_MATCHES), // Rapido-style job offers pushed to the worker
...
export const authAPI = {
  signup: async ({ name, email, password, type, city, area, bizCategory }) => {
    ...
    const user = {
      ...
      area: area || "", bizCategory, bio: "", skills: "", skillGraph: [], skillMeta: {}, phone: "",
...
// ── MATCHES (worker side — Rapido-style pushed job offers) ────────────────────
// Each offer is scored against the signed-in worker's own skill graph, so the
// match % and reasons are personal. The worker accepts (creates an application)
// or skips; either way nobody is left hanging.
function scoreMatch(offer) {
  const u = meUser() || {};
  const shape = {
    skilledIn: verifiedSkillKeys(u),
    openTo: openToKeys(u),
    openToLearn: false,
    distKm: offer.distKm,
    reliability: 90,
    catRating: u.rating || 4.6,
    expectRate: null,
  };
  const c = compatibility(shape, offer);
  return { ...offer, matchPct: c.score, matchReasons: c.reasons };
}

export const matchesAPI = {
  // Open offers only, best match first.
  getForMe: async () => {
    await ensure(); await delay();
    db.matches = db.matches || clone(seed.SEED_MATCHES);
    const open = db.matches.filter((m) => m.status === "offered").map(scoreMatch);
    open.sort((a, b) => b.matchPct - a.matchPct);
    return clone(open);
  },
  countForMe: async () => {
    await ensure();
    db.matches = db.matches || clone(seed.SEED_MATCHES);
    return db.matches.filter((m) => m.status === "offered").length;
  },
  // Accepting an offer creates a real application on the worker's tray and
  // notifies them, then closes the offer.
  accept: async (id) => {
    await ensure(); await delay(200);
    const m = db.matches.find((x) => x.id === id);
    if (!m || m.status !== "offered") return null;
    m.status = "accepted";
    const now = Date.now();
    db.applications.unshift({
      id: uid("a"), gigId: m.id, gigTitle: m.gigTitle, who: m.who, initials: m.initials,
      area: m.area, payAmount: m.payAmount, payUnit: m.payUnit, hrs: m.hrs,
      status: "applied", appliedAgo: "now", note: "",
      appliedAt: now, respondBy: now + RESPOND_WINDOW_HRS * HOUR,
    });
    db.notifications.unshift({ id: uid("n"), type: "applied", title: "You accepted a match", body: `You're in for ${m.gigTitle} at ${m.who}. ${m.who} will confirm within ${RESPOND_WINDOW_HRS}h.`, postedAgo: "now", read: false });
    await persist();
    return clone(m);
  },
  skip: async (id) => {
    await ensure(); await delay(150);
    const m = db.matches.find((x) => x.id === id);
    if (m && m.status === "offered") { m.status = "skipped"; await persist(); }
    return clone(m);
  },
};
