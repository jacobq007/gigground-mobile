...
export default function Dashboard() {
  ...
  const active = (apps || []).filter((a) => a.status !== "done");
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={["top"]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 28 }} showsVerticalScrollIndicator={false}>
        ...
        {!apps ? <Skeleton height={120} /> : active.length === 0 ? (
          ...
          return (
            <Pressable key={a.id} onPress={() => a.status === "confirmed" && router.push(`/modals/active-gig?id=${a.id}`)} style={s.card}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 5 }}>
                ...
                {a.status === "confirmed" ? <Text style={s.openTxt}>Open gig <Ionicons name="chevron-forward" size={11} /></Text> : null}
              </View>
            </Pressable>
          );
        })}
