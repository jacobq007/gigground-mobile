...
async function ensure() {
  ...
  if (!db) {
    db = {
      ...
      applicants: {}, // gigId → applicants on gigs the current user posted
...
function applicantsFor(gigId) {
  ...
  if (!db.applicants[gigId]) {
    ...
    db.applicants[gigId] = pool.slice(0, n).map((p, i) => ({
      id: uid("ap"), gigId, status: "applied", appliedAgo: `${(i + 1) * 9}m`,
      initials: p.name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase(),
      etaMin: Math.round(p.distKm * 6 + 4), ...p,
    }));
  }
  return db.applicants[gigId];
}

export const applicantsAPI = {
  // Sorted by fit score; the algorithmic best carries topPick (hirer can ignore it).
  getForGig: async (gigId) => {
    await ensure(); await delay();
    const list = applicantsFor(gigId);
    await persist();
    const out = clone(list).sort((a, b) => fitScore(b) - fitScore(a));
    const best = out.find((a) => OPEN_APP_STATUSES.includes(a.status));
    out.forEach((a) => { a.fit = fitScore(a); a.topPick = best ? a.id === best.id : false; });
    return out;
  },
  ...
  shortlist: async (id) => {
    await ensure(); await delay(200);
    for (const list of Object.values(db.applicants)) {
  ...
  // Hiring one applicant instantly auto-declines everyone else on the gig —
  // no ghosting-by-default. Returns how many were auto-declined.
  hire: async (id) => {
    await ensure(); await delay(200);
    for (const list of Object.values(db.applicants)) {
      const a = list.find((x) => x.id === id);
